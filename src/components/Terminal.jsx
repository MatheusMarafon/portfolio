...
const Terminal = ({ isOpen, onClose }) => {
  ...
  const handleCommand = async (cmd) => {
    ...
    if (cmd === 'help()' || cmd === 'dir()') {
        ...
        newHistory.push({ type: 'response', content: "Matheus Marafon, 22 anos." });
        newHistory.push({ type: 'response', content: "Engenheiro de Software focado em resolver problemas reais." });
        newHistory.push({ type: 'response', content: "Especialista em Python, Automação e Dados." });
    }

    else if (cmd === 'print(skills)') {
        newHistory.push({ type: 'success', content: "['Python', 'Django', 'Selenium', 'Pandas', 'SQL', 'React', 'VBA']" });
    }
